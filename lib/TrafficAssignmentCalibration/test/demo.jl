...
dfw = DiffFW(
    ...
    step_size=1e-5,
    implicit_kwargs=(;
        representation=MatrixRepresentation(), linear_solver=DirectLinearSolver()
    ),
)
...
plot_network(problem, flow; edge_quantity_type=:flow)
plot_network(problem, capa_grad; edge_quantity_type=:gradient)
